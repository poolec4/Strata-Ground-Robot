...
class World:
	def __init__(self, depth_map, local_start, world_size=[100, 100]):
	...
	def coords2grid(self, x, y, local_start=[0,0]):
		x = int(x/self.grid_size[0]-0.5)+local_start[0]
		y = int(y/self.grid_size[1]-0.5)
		return  x, y

	def coords2grid2(self, x, y):
	...
	def addBuffer(self): # Adds buffer around obstacle
		...
		inner_buff_width = int(round(0.3/self.grid_size[0]))  # > half of robot's width in grids
		outer_buff_width = int(round(0.5/self.grid_size[0]))  # ~robot's width in grids
		for i in range(self.world_size[0]):
			for j in range(self.world_size[1]):
						...
						for m in range(j-1, self.world_size[1]):
							if k >= 0 and k < self.world_size[0]:
								if abs(k-i) <= inner_buff_width:
									discounted_height = height
								else:
									discounted_height = height/abs(k-i)
								if self.world[k, m] < discounted_height:
									new_world[k, m] = discounted_height
...
def coords2enc(coords, world, local_goal=[0,0]): # returns encoded grid value
	print('coordinates: ',coords)
	grid_coords = world.coords2grid(coords[0], coords[1], local_goal)
	grid_enc = world.enc_world[grid_coords[0]][grid_coords[1]]
	print('encoded: ', grid_enc)
	return grid_enc
...
def path2coords(path, world, grid_size, local_start, global_goal):
	x_coords = []
	y_coords = []
...
def getLocalGoal(global_start, global_goal, global_angle, world, local_start):
	...
	if dist_to_goal[0] <= world.bounds[0, 1] and dist_to_goal[1] <= world.bounds[1, 1]: # goal in current world
		...
		# print(offset)
		print('Goal in grid')
		P_diff = np.asarray(P_g)-np.asarray(P_v)
		print('P_diff: ', P_diff)
		local_coords = np.matmul(R_gl, P_diff)  #-np.matmul(R_gl, P_g)).flatten()  #-global_start
		print('local goal coords', local_coords)
		print('local start coords ', local_start)
		print('local start dist: ', coords2dist(local_start, world.grid_size, local_start))
		...
		local_coords = [(world.bounds[0, 1]-world.bounds[0, 0])*math.cos(shortest_path_angle), (world.bounds[1, 1]-world.bounds[1, 0])*math.sin(shortest_path_angle)]
		print('local coords ', local_coords)
	return local_coords
...
def plan(start, dest, depth_map, world): # Called to plan trajectory
	...
	dest_enc = coords2enc(dest, world, start)
	print('local dest: ', dest)
	print('dest enc ', dest_enc)
	path_obj = shortest_path(graph, start_enc, dest_enc)
	path_cost = path_obj[0]
	path = path_obj[1]
	x_coords, y_coords, grid_coords = path2coords(path, world, world.grid_size, start, dest)
